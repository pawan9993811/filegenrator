                            ...
                            if (insertNow)
                            {
                                for (int j = i; j < parents.Count; j++)
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(aEl.ClassName))
                                    {
                                        ...
                                        propchildName.SetAttribute("ClassName", aEl.ClassName);
                                        searchCritchild.AppendChild(propchildName);
                                    }
                                    if (!string.IsNullOrEmpty(aEl.LocalizedControlType))
                                    {
                                        XmlElement propchildName = doc.CreateElement("Property");
                                        propchildName.SetAttribute("ControlType", aEl.LocalizedControlType);
                                        searchCritchild.AppendChild(propchildName);
                                    }
                                    if (!string.IsNullOrEmpty(aEl.Name))
                                    {
                                        XmlElement propchildName = doc.CreateElement("Property");
